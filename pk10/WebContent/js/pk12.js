...
var initCompareTable = function(startDay, endDay) {
	if (!($.trim(startDay).length > 0 && $.trim(endDay).length > 0)) alert("initCompareTable 日期参数有误。");
...
//		var mainCorrectIndex = -1;	//主道猜对位置
		...
		if (k <= compareDay[1].length - 3) {	//只处理到倒数第三条
			var numbersArr = compareDay[1][k][1];
			for (var x=0; x<numbersArr.length; x++) {
				if (numbersArr[x] == compareDay[1][k+1][1][x] && numbersArr[x] == compareDay[1][k+2][1][x]) {	// 出现三连
//					t++;
					pairIndex = x;
//					thirdPairRow = k + 2;
					thirdPairRowArr.push([k + 2, numbersArr[x]]);
...
//			if (thirdPairRow > -1) {
			if (thirdPairIndex > -1) {
				...
				var inde = -1;
				for (var b=0; b<carsList.length; b++) {
					if (mainDay[1][k][1][thirdPairIndex] == carsList[b]) {
						inde = b;						
					}
				}
				if (inde > -1) {	// 查找主道对应位置是否出现追车队列中的车号
...
//					mistakeNum++;
		...
		for (var d=0; d<compareDay[1][k][1].length; d++) {
			...
			if (d == pairIndex)	//当前一连所在位置
				htm = "<td><span class='mainCarBGColor'>" + compareDay[1][k][1][d] + "</span></td>";
			if (d == nextPairIndex)	//当前二连所在位置
				htm = "<td><span class='mainCarBGColor'>" + compareDay[1][k][1][d] + "</span></td>";
			if (d == thirdPairIndex)	//当前三连所在位置
				htm = "<td><span class='mainCarBGColor'>" + compareDay[1][k][1][d] + "</span></td>";
			numbersHtml += htm;
		}
...
//			var ind = -1;
//			for (var t=0; t<thirdPairRowArr.length; t++) {
//				if (k == thirdPairRowArr[t][0]) {
//					ind = t;
//					break;
//				}
//			}
