		...
		(
				...
				function(root, daterangepicker, moment, $) {
					var DateRangePicker = function(element, options, cb) {
						// by default, the daterangepicker element is placed at
						// the bottom of HTML body
						this.parentEl = 'body';
						...
						if (typeof options !== 'object' || options === null)
							options = {};
						this.parentEl = (typeof options === 'object'
								&& options.parentEl && $(options.parentEl).length) ? $(options.parentEl)
								: $(this.parentEl);
						...
						this.container.find(
								'.daterangepicker_start_input label').html(
						...
						this.container
								...
								.on(
										...
										'.daterangepicker_start_input,.daterangepicker_end_input',
										$.proxy(this.inputsKeydown, this)).on(
...
DateRangePicker.prototype = {
						...
						setOptions : function(options, callback) {
							...
							if (typeof options.locale === 'object') {
								if (typeof options.locale.daysOfWeek === 'object') {
									// Create a copy of daysOfWeek to avoid
									// modification of original
									// options object for reusability in
									// multiple daterangepicker instances
							...
							if (this.locale.firstDay != 0) {
								...
								while (iterator > 0) {
									this.locale.daysOfWeek
											.push(this.locale.daysOfWeek
													.shift());
									iterator--;
								}
							}
							var start, end, range;
							// if no start/end dates set, check if an input
							// element contains initial values
							...
							if (typeof options.ranges === 'object') {
								for (range in options.ranges) {
									...
									// If we have a min/max date set, bound this
									// range
									// to it, but only if it would otherwise
									// fall
									...
									// If the end of the range is before the
									// minimum (if min is set) OR
									// the start of the range is after the max
									// (also if set) don't display this
									// range option.
									if ((this.minDate && end
							...
							if (this.singleDatePicker) {
								...
								if (!this.container.find('.calendar.right')
										.hasClass('single'))
							...
							this.leftCalendar = {
								month : moment([ this.startDate.year(),
							...
							this.rightCalendar = {
								month : moment([ this.endDate.year(),
							...
							if (this.opens == 'right') {
								...
								var left = this.container
										.find('.calendar.left');
								var right = this.container
										.find('.calendar.right');
						...
						updateView : function() {
							this.leftCalendar.month.month(
									...
									this.startDate.year()).hour(
									this.startDate.hour()).minute(
									this.startDate.minute());
							this.rightCalendar.month
									.month(this.endDate.month()).year(
											this.endDate.year()).hour(
											this.endDate.hour()).minute(
											this.endDate.minute());
						...
						updateFromControl : function() {
							...
							var dateString = this.element.val().split(
									this.separator), start = null, end = null;
...
this.container.css({
						...
						hide : function(e) {
							if (!this.isShowing)
								return;
							$(document).off('mousedown.daterangepicker').off(
									'click.daterangepicker',
						...
						// when a date is typed into the start to end date
						// textboxes
						...
						setCustomDates : function(startDate, endDate) {
							...
							if (startDate.isAfter(endDate)) {
								var difference = this.endDate
										.diff(this.startDate);
								endDate = moment(startDate).add(difference,
										'ms');
							}
						...
						clickDate : function(e) {
							...
							if (cal.hasClass('left')) {
								...
								if (typeof this.dateLimit === 'object') {
									var maxDate = moment(startDate).add(
											this.dateLimit).startOf('day');
								...
								if (typeof this.dateLimit === 'object') {
									var minDate = moment(endDate).subtract(
											this.dateLimit).startOf('day');
						...
						updateMonthYear : function(e) {
							...
							this[leftOrRight + 'Calendar'].month.month(month)
									.year(year);
							this.updateCalendars();
						},
						...
						updateCalendars : function() {
							...
							this.container.find('.calendar.left').empty().html(
									this.renderCalendar(
						...
						buildCalendar : function(month, year, hour, minute,
							...
							var curDate = moment([ lastYear, lastMonth,
									startDay, 12, minute ]);
							var col, row;
							for (i = 0, col = 0, row = 0; i < 42; i++, col++, curDate = moment(
									curDate).add(24, 'hour')) {
								if (i > 0 && col % 7 === 0) {
									col = 0;
									row++;
						...
						renderCalendar : function(calendar, selected, minDate,
								maxDate) {
							...
							if (this.showDropdowns) {
								dateHtml = this.renderDropdowns(calendar[1][1],
										minDate, maxDate);
							}
							...
							for ( var row = 0; row < 6; row++) {
								...
								for ( var col = 0; col < 7; col++) {
									...
									cname += (calendar[row][col].month() == calendar[1][1]
											.month()) ? '' : 'off';
									if ((minDate && calendar[row][col]
											.isBefore(minDate, 'day'))
											|| (maxDate && calendar[row][col]
													.isAfter(maxDate, 'day'))) {
										cname = ' off disabled ';
									} else if (calendar[row][col]
											.format('YYYY-MM-DD') == selected
											.format('YYYY-MM-DD')) {
										cname += ' active ';
										if (calendar[row][col]
